...
class ConversationEngine:
    ...
    async def send_prompt(self, prompt):
        ...
        if constants.COMMANDS_KEY in response:
            self._commands = response[constants.COMMANDS_KEY]

        if constants.PROBLEM_KEY in response:
            self._problems = response[constants.PROBLEM_KEY]
        else:
            self._problems = None
